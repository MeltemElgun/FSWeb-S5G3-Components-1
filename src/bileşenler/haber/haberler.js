...
const haber = document.querySelector(".articles");
function haberYap(dataA) {
  const { baslik, tarih, ilkParagraf, ikinciParagraf, ucuncuParagraf } = dataA;

  let haberYapici = document.createElement("div");
  haberYapici.className = "article";

  let haberHeading = document.createElement("h2");
  ...
  haberYapici.appendChild(haberSpan);
  return haberYapici;
}
for (let i = 0; i < data.length; i++) {
  haber.appendChild(haberYap(data[i]));
}
